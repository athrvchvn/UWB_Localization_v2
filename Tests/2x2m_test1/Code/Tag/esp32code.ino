...
// 5 m (X) × 7 m (Y) right-triangle layout
float anchor_matrix[N_ANCHORS][3] = {
    {0.0f, 0.0f, 0.0f}, // Anchor #1 (0x84) — origin
    {0.0f, 7.0f, 0.0f}, // Anchor #2 (0x85) — 7 m along Y
    {5.0f, 0.0f, 0.0f}, // Anchor #3 (0x86) — 5 m along X
};
...
void newRange() {
  ...
  if (index > 0) {
    ...
    // hard sanity gate — 5x7 m diagonal ≈ 8.60 m; 12 m gives headroom
    if (raw < 0.0f || raw > 12.0f)
      return;
